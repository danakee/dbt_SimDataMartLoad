...
{% materialization dim_project, adapter='sqlserver' %}
  ...
  {% if existing_relation is none %}
      ...
      {% call statement('main') -%}
          SELECT * INTO {{ target_relation }} FROM {{ temp_relation }};
          ALTER TABLE {{ target_relation }} ADD ProjectKey int IDENTITY(1,1) CONSTRAINT PK_DimProject PRIMARY KEY;
          ALTER TABLE {{ target_relation }} ADD EDWCreatedDatetime datetime CONSTRAINT DF_DimProject_EDWCreatedDatetime DEFAULT (getdate());
          ALTER TABLE {{ target_relation }} ADD EDWLastUpdatedDatetime datetime CONSTRAINT DF_DimProject_EDWLastUpdatedDatetime DEFAULT (getdate());
      {%- endcall %}
  {% else %}
      -- Incremental update
      {% call statement('main') -%}
          ...
          WHEN MATCHED THEN
              UPDATE SET
                  ...
                  EDWLastUpdatedDateTime = GETDATE()
